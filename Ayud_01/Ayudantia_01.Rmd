...
rep("1",4)
rep(c(1,5,8,2),4)
rep(c(1,2,"hola",2),4) #transformó todo a string
```
...
#titanic::una_base_de_datos -> los dos dos puntos me indican las bases de datos/funciones que tiene el paquete
```
...
Titanic[Pclass == 1 | Pclass == 3,] # hace lo mismo que arriba
...
```{r Viendo base}
View(Titanic)
head(Titanic,5)  #Alternativa - por default son las primeras 6, pero ahora puse 5
nrow(Titanic) #me dice cuantas filas hay
ncol(Titanic) #cuantas variables (columnas) hay
```

¿Que clase de objeto es dicha base de datos? ¿Y cada una de sus variables?

```{r Clase}
class(Titanic) ## Antes de trabajar con una base de datos, hay que revisar las clases
class(Titanic[,Name]) ## Podría ser de clase factor
...
```{r Categorías}
unique(Titanic[,Embarked])  ## intentar con función "levels" para el caso de columnas de clase "factor"    ---> en este caso hay un vacío que no es un N/A sino que un espacio " "
table(Titanic[,Embarked]) ## cuántas veces está cada categoría en la columna
unique(Titanic[,Sex])
...
length(unique(Titanic[,Name])) #no se repite ningún nombre
```
...
```{r summary}
summary(Titanic) ##Lo mismo de antes pero para todas las variables y número de N/A
```
